...
- **Без цензуры.** Локальная текстовая модель плюс abliterated пайплайн FLUX — ничем не ограниченное взрослое повествование и арт, целиком на твоё усмотрение и полностью приватно.
...
- Игровой режим включается на каждый чат — врубай его для полноценной RPG-сессии или оставь выключенным для свободного повествования.
...
- Abliterated текст-энкодер снимает контентный фильтр для ничем не ограниченного арта 18+.
...
- **Русскоязычный интерфейс** — весь UI локализован (промпты картинок остаются на английском для FLUX).
...
- **GPU:** NVIDIA, 12+ ГБ VRAM (RTX 40xx/50xx полностью поддержаны; 20xx/30xx/Pascal выбираются в установщике). Установщик ставит подходящие CUDA-колёса (cu118 / cu126 / cu128) под твой GPU.
...
- **Веса моделей, которые ты предоставляешь / докачиваешь при первом запуске:** локальная текстовая GGUF-модель, веса картинок FLUX.2-klein + расцензуренный текст-энкодер, модель ASR Parakeet и голоса TTS. В репозитории ничего не лежит — укажи лаунчеру путь к весам, и рантаймы докачают недостающее при первом старте.
