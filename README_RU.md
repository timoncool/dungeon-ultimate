...
- **Веса моделей — всё качается само, ничего предоставлять руками не нужно.** `install.bat` клонирует два бэкенда (TTS, картинки) и скачивает голосовой пак в `servers/voices/`; при первом старте все модели тянутся с Hugging Face — Gemma 4 12B GGUF (текст), веса картинок FLUX.2-klein SDNQ, голосовая модель Qwen3-TTS, модель ASR Parakeet. Всё это негейтнутое. Полностью abliterated текст-энкодер FLUX лежит в **гейтнутом** HF-репо, поэтому остаётся опциональным: укажи `IMAGE_SERVER_DEFAULT_BACKEND=flux-uncensored`, когда получишь доступ к репо и HF-токен. Можешь также положить свои `.mp3`-клипы в `servers/voices/`.
