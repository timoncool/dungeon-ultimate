...
- **Веса моделей — всё качается само при первом запуске:** ничего предоставлять руками не нужно. `install.bat` ещё и клонирует два бэкенда (TTS, картинки); при первом старте все модели тянутся с Hugging Face — Gemma 4 12B GGUF (текст), веса картинок FLUX.2 SDNQ + расцензуренный текст-энкодер, модель ASR Parakeet. Опционален только голосовой пак для TTS — положи `.mp3`-клипы в `servers/voices/` для озвучки вслух.
