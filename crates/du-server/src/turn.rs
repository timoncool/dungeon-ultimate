...
pub fn engine_schema() -> Value {
    ...
    json!({
        ...
        "properties": {
            ...
            "note": {
                "type": "string",
                "description": "короткая заметка ИГРОКУ о последствии хода: что изменилось в мире.                     НЕ рассуждение, не план, не разбор правил и НИКОГДА не исход проверки — исход                     решает движок. Про задания, поручения и броски НЕ ПИШИ ВООБЩЕ: у них свои                     события, а объяснения вида «поручения не последовало» игроку не нужны",
                "maxLength": 160
            }
        }
    })
