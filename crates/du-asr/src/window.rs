...
pub fn merge_windows(atoms: &[(f64, f64)], total: f64, cfg: &WindowConfig) -> Vec<Window> {
    ...
    // Добавить захлёст и заклампить в [0, total]: конец расширяем на overlap (кроме последнего окна),
    // начало — тоже (кроме первого); дубли слов на швах снимает де-дуп ASR.
    let n = raw.len();
